...
const OrderData = ({ index, data, admin }) => {
  ...
  return <motion.div 
    ...
    className="w-full flex flex-col items-start justify-start px-3 py-2 border relative border-gray-300  bg-[rgba(256,256,256,0.4)] drop-shadow-lg rounded-lg gap-4"> 
        <div className="w-full flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
                ...
                <p className="px-2 py-[2px] txt-sm text-headingColor font-semibold capitalize rounded-md bg-emerald-400 drop-shadow-lg">
                  {data?.status}  
                </p>

                <p className={`text-base font-semibold capitalize border border-gray-300 px-2 py-[2px] rounded-md shadow-md ${
            ...
            <div className="flex items-center justify-center gap-4">
                {data?.items && data.items.map((item,j) => (
                    ...
                    className="flex items-center justify-center gap-1">
                        <img
                            src={item.imageURL}
                            className="w-[150px] h-15 object-contain"
                            alt=""
                        />
